...
public class SearchEngineServiceImpl implements SearchEngineService {
    ...
    private final Lemmatizer morphology;
    ...
    @Override
    public List<SearchPageResult> allSiteSearch(String searchText, int offset, int limit) {
        log.info("Получаем инфомацию по поиску \"" + searchText + "\"");
        var siteList = siteRepository.findAll();
        List<SearchPageResult> result = new ArrayList<>();
        ...
        List<SearchPageResult> searchData = new ArrayList<>(getSearchDtoList(foundLemmaList, textLemmaList, offset, limit));
    ...
    @Override
    public List<SearchPageResult> siteSearch(String searchText, String url, int offset, int limit) {
    ...
    private List<SearchPageResult> getSearchDtoList(List<Lemma> lemmaList,
    ...
    private LinkedHashMap<PageEntity, Float> getPageAbsRelevance(List<PageEntity> pageList, List<IndexEntity> indexList) {
    ...
    private List<SearchPageResult> getSearchData(LinkedHashMap<PageEntity, Float> pageList,
                                          List<String> textLemmaList) {
        List<SearchPageResult> result = new ArrayList<>();
...
//            var pageSite = page.getSite();
            var pageSite = siteRepository.findById(page.getSiteId()).orElse(new Site());
            ...
            result.add(new SearchPageResult(site,
                    siteName,
                    uri,
                    title,
                    snippet,
